...
| **US03** : As a new user, I want to create an account with my email and password, so I can leave comments and like/dislike anime or comments. | Verify users can register an account | 1. Users can access a sign-up page from the homepage.  2. The form includes fields for email, username, and password (with password confirmation).  3. The system verifies the email's uniqueness and displays an error if the email is already registered. | Users can find the register button from the home page. Users can enter a username Email(optional) Password and password comfirmation. If user tries using an already in use email give an error. | Users can find the register button on the homepage. Users can choose Username, add email, make a password and confirm password. I am now having an issue with the signup not working though it was working fine before I submitted on the 14th October as I made and account just before to test it If a user uses an already in-use email doesn't give an error or text but doesnt let the account create. | 1. Pass 2. Fail 3. Fail  |
| **US04** : As a returning user, I want to log in to my account, so I can interact with the content (comment, like/dislike). | Verify returning user can login to account made previously | | 1. Users can access a login page from the homepage.  2. Users are redirected to the homepage upon successful login.  3. Users are shown an error message if the credentials are incorrect. | When Users come to the website they can click the login button and enter there Username and Password to login. If incorrect give an error | If a Users types incorrect details it gives an error text at top of form | Pass |
| **US05** : As an admin, I want to manage user accounts and interactions, so I can moderate content for the website. | Verify that an admin user can view and delete Users | 1. Admins can see a list of all registered users.  2.Admins can deactivate, edit, or delete user accounts. | Admin Users can view, edit and delete Users account/profile | Admins are able to modify or delete Users account | Pass |
| **US06** : As an admin, I want to moderate user comments and delete inappropriate or harmful content, so I can ensure the site remains a positive space. | Verify that admins can view, aprrove, change and delete users comments | 1. Admins can access a separate "Admin Dashboard" from the navbar or URL.  2. The dashboard displays a list of all comments on the site.  3. Admins can search for comments by username, anime title, or keywords.  4. Admins can delete any inappropriate comment (with confirmation to prevent accidental deletion).  5. Admins can view details about the user who posted the comment (username, email, date of registration). | Admins can view, aprrove, change and delete users comments | Admins have a separate dashboard they can get to by URL but no button on navbar. The dashboard shows users, comments, posts. Not able to search comments by Users,anime title, or keywords. Admins can delete comments. Admins can see who posted the comments | 1. Fail 2. Pass 3. Fail 4. Pass 5. Pass |
| **US07** : As a logged-in user, I want to comment on my favorite anime from the list, so I can share my thoughts with other fans. | Verify Users can post comments | 1. Logged-in users can see a comment box below the anime details.  2. Comments display the username and timestamp of when they were posted.  3. Users can delete or edit their own comments.  4. Logged-out users see a prompt to "Log in to comment."  5. The comment list should display in reverse chronological order (latest comment at the top). | Users can see the lastest comments and post new comments when logged in. If not logged in give message saying you need to be logged in to leave a comment. | Users can see and post new comments if they are logged in, If they are not logged in sees a message saying log in to post a comment | Pass |
